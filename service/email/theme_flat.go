          ...
          <tr>
            <td class="email-masthead">
              <a class="email-masthead_name" href="{{.Product.Link}}" target="_blank">
                {{ if .Product.Logo }}
                  <img src="{{.Product.Logo | url }}" class="email-logo" />
                {{ else }}
                  {{ .Product.Name }}
                {{ end }}
                </a>
              ...
              <table class="email-body_inner" align="center" width="570" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td class="content-cell">
                    <h1>{{if .Body.Title }}{{ .Body.Title }}{{ else }}{{ .Body.Greeting }} {{ .Body.Name }},{{ end }}</h1>
                    {{ with .Body.Intros }}
                        {{ if gt (len .) 0 }}
                          {{ range $line := . }}
                            <p>{{ $line }}</p>
...
func (dt *Flat) PlainTextTemplate() string {
	return `<h2>{{if .Body.Title }}{{ .Body.Title }}{{ else }}{{ .Body.Greeting }} {{ .Body.Name }}{{ end }},</h2>
{{ with .Body.Intros }}
  {{ range $line := . }}
    <p>{{ $line }}</p>
  {{ end }}
...
{{ with .Body.Outros }} 
  ...
  {{ end }}
{{ end }}
<p>{{.Body.Signature}},<br>{{.Product.Name}} - {{.Product.Link}}</p>
